    ...
    rendering() {
        ...
        if (this.paths.length === 0) {
            ...
            this.paths.forEach(pathObject => {
                this.ctx.strokeStyle = pathObject.strokeColor;
    ...
    handleMouseMove(event) {
        ...
        this.paths.forEach(path => {
            if (this.ctx.isPointInStroke(path.path, x, y)) {
                path.strokeColor = this.hoverColor;
                path.fillColor = this.hoverColor;
            } else {
                path.strokeColor = this.defaultColor;
                path.fillColor = this.defaultColor;
            }
        });
    },
    drawTile({ x, y }) {
...
function createTile(startPoint) {
    ...
    shapes.forEach(shape => {
        ...
        this.paths.push({
            path,
            strokeColor: shape.strokeColor || this.defaultColor,
            fillColor: shape.fillColor || this.defaultColor,
        });
    });
}
